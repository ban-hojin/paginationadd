// 1. 처음과 맨 끝 버튼 추가 
// 1.1 paginationHTML 변수 지정
// 1.2 for문 사용 : for (let i = first; i <= last; i++)
// 1.3. 온클릭 이벤트 발생 onclick="pageClick(${i})" >${i}
// 1.4  현출 : paginationHTML +=

// 2. 처음과 맨 끝 페이지그룹에서는 화살표 없애기
// 2.1 6보다 크거나 같을 경우 함수 if (first >= 6) {}
// 2.2 for (let i = first; i <= last; i++) 함수 발생
// 2.3 if (last < totalPage) 함수 발생 

// 3. 페이지수가 5이하일 경우 또는 마지막 페이지 그룹이 5이하일 경우 
// 3.1 let last = pageGroup * 5;  if (last > totalPage) last = totalPage
// 3.2 let first = last - 4 <= 0 ? 1 : last - 4
